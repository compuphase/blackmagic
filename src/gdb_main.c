...
static void handle_q_packet(char *packet, size_t plen);
static void handle_v_packet(char *packet, size_t plen);
...
static bool exec_command(char *packet, size_t length, const cmd_executer *exec)
{
    while(exec->cmd_prefix) {
        size_t prefix_length = strlen(exec->cmd_prefix);
...
static void exec_qRcmd(const char *packet, size_t length)
{
    /* calculate size and allocate buffer for command */
    size_t datalen = length / 2;
    char *data = alloca(datalen+1);
    /* dehexify command */
    unhexify((uint8_t*)data, packet, datalen);
    ...
    if(c < 0) {
        ...
        gdb_putpacket(hexify(pbuf, (const uint8_t*)msg, strlen(msg)), 2 * strlen(msg));
    }
}

static void handle_q_string_reply(const char *str, const char *param)
{
    size_t reply_length = strlen(str);
    uint32_t addr = 0;
    uint32_t len = 0;
...
static void exec_qSupported(const char *packet, size_t length)
{
    (void)packet;
    (void)length;
...
static void exec_qMemoryMap(const char *packet, size_t length)
{
    (void)length;
...
static void exec_qFeatureRead(const char *packet, size_t length)
{
    (void)length;
...
static void exec_qCRC(const char *packet, size_t length)
{
    (void)length;
    uint32_t addr, alen;
 ...
 */
static void exec_qC(const char *packet, size_t length)
{
    (void)packet;
    (void)length;
    gdb_putpacketz("QC1");
 ...
 */
static void exec_qThreadInfo(const char *packet, size_t length)
{
    (void)length;
    if (packet[-11] == 'f')
        gdb_putpacketz("m1");
...
static const cmd_executer q_commands[] =
{
    ...
    {"qCRC:",                          exec_qCRC},
    {"qC",                             exec_qC},
    {"qfThreadInfo",                   exec_qThreadInfo},
    {"qsThreadInfo",                   exec_qThreadInfo},
    {NULL,NULL},
};
...
static void handle_q_packet(char *packet, size_t plen)
{
    if(exec_command(packet, plen, q_commands))
        return;
    DEBUG_GDB("*** Unsupported packet: %s\n", packet);
    gdb_putpacket("", 0);
}
...
static void handle_v_packet(char *packet, size_t plen)
{
        ...
        *cmdline = '\0';
        while(*tok != '\0') {
            ...
            if (isxdigit(*tok) && isxdigit(*(tok+1))) {
                unhexify((uint8_t*)pcmdline, tok, 2);
    ...
    } else if (strcmp(packet, "vFlashDone") == 0) {
        /* Commit flash operations. */
        if (target_flash_done(cur_target))
            gdb_putpacketz("OK");
        else
            gdb_putpacketz("EFF");
        flash_mode = 0;

    } else if (strcmp(packet, "vStopped") == 0) {
